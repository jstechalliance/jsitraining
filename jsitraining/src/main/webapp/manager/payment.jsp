												...
												out.println("<div id='"+count+"' style='display:none' >");
													...
													int pdCount=0;
														for(Payment payment:paymentDetailList){
															...
															out.println("<input type='hidden' value='"+employeeService.getEmployeeDetailByEmployeeId(new Employee(payment.getReceived_by())).getFull_name()+"' id='previous_id_receiverName"+(count+pdCount)+"'>");
															...
															out.println("<td width='12%' align='center' id='p_approve_id_"+(count+""+pdCount)+"'>");
																if(payment.getPayment_status().equalsIgnoreCase("Not Approved")){
																	out.println("<a href='#' id='approveLink"+(count+pdCount)+"' onclick='approvePayment(\""+payment.getPayment_id()+"\",\""+(count+pdCount)+"\",\""+count+"\",\""+(count+""+pdCount)+"\");'>"+payment.getPayment_status()+"</a>");
																}else{
												...
												for(AcademicProjectDTO academicProjectDTO:searchResultListForPayment){
													...
													int pdCount=0;
														for(Payment payment:paymentDetailList){
															...
															out.println("<input type='hidden' value='"+payment.getMyid()+"' id='previous_myid"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+payment.getSource()+"' id='previous_source"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+academicProjectDTO.getProject_name()+"' id='previous_cdetail"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+payment.getPayment_mode()+"' id='previous_id_payment_mode"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+enquiryDTO.getAcademic_project_enquiry_fullname()+"' id='previous_id_candidateName"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+employeeService.getEmployeeDetailByEmployeeId(new Employee(payment.getReceived_by())).getFull_name()+"' id='previous_id_receiverName"+(count+pdCount)+"'>");
															out.println("<input type='hidden' value='"+payment.getAmount()+"' id='previous_my_id_amount"+(count+pdCount)+"'>");
